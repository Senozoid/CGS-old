...
class OutputController {
  private:
    ...
    double spath_query_time;
    double compressed_spath_query_time;
    double decompressed_spath_query_time;
    double decompression_time;
    
    void shortest_path_query(int k);
    void spath_times();
    void find_path();
...
OutputController::OutputController(char* dataset_, char* compressed_EL_filename_, char* compressed_SN_filename_, Graph original, CompressedGraph compressed, char* algo_, char* outfile) {
  ...
  filename = outfile;
  spath_query_time = 0;
  compressed_spath_query_time = 0;
  decompressed_spath_query_time = 0;
...
void OutputController::shortest_path_query(int k) {
  clock_t start, end;
  ...
  REP(i, G.n) {
    G_d.adj[i] = G_.NQ(i);
    G_d.m += (int) G_d.adj[i].size();
  }
  G_d.m /= 2;
  G_d.size = G_d.n + (2*G_d.m);
  end = clock();
  decompression_time = double(end - start) / double(CLOCKS_PER_SEC);
  ...
  REP(i, k){
    ...
    spath_query_time += double(end - start) / (double(CLOCKS_PER_SEC));

    // On compressed graph (by local decompression)
    start = clock();
    int path_G_ = G_.SP(node1, node2);
    end = clock();
    compressed_spath_query_time += double(end - start) / (double(CLOCKS_PER_SEC));

    // On completely decompressed graph
    start = clock();
    int path_G_d = G_d.SP(node1, node2);
    end = clock();
    decompressed_spath_query_time += double(end - start) / (double(CLOCKS_PER_SEC));

    // Computing error in shortest path query
    if(path_G > 0) {
      double e = abs(path_G - path_G_)/(1.0 * path_G);
      error.push_back(e);
      avg += e;
      maxx = max(maxx,e);
      if(error.size()==1) minn = e;
      minn = min(minn, e);
    }
  }

  spath_query_time = spath_query_time /(1.0 * k);
  compressed_spath_query_time = compressed_spath_query_time /(1.0 * k);
  decompressed_spath_query_time = decompressed_spath_query_time /(1.0 * k);

  avg = avg/ (1.0 * error.size()); // Compute average
  // Computing standard deviation
  REP(i, error.size()) std += pow((error[i] - avg),2);
  std = sqrt(std/(1.0 * error.size()));
  ...
  cout<<"\n\nShortest path query results on "<< k <<" random node pairs:";
  cout<<"\n----------------------------------------------------";
  cout<<"\nAverage error in shortest path: "<< avg;
  cout<<"\nMinimum error in shortest path: "<< minn;
  cout<<"\nMaximum error in shortest path: "<< maxx;
  cout<<"\nStandard deviation of error in shortest path: "<< std;
}
...
void OutputController::spath_times() {
  cout<< "\nShortest path query time on G (in seconds): "<< spath_query_time;
  cout<< "\nShortest path query time on G_c (in seconds): "<< compressed_spath_query_time;
  cout<< "\nDecompression time of G_c (in seconds): "<< decompression_time;
  cout<< "\nShortest path query time on globally decompressed G_c (in seconds): "<< decompressed_spath_query_time;
...
void OutputController::print_analysis() {
  ...
  G_.get_parent();
  shortest_path_query(100000); // computes shortest path query for 100000 node pairs
  spath_times();
  cout << "\n\n"; 
